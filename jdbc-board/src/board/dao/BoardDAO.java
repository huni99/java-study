...
//				board.setRegDate(rs.getTimestamp("reg_date").toLocalDateTime());
	...
	public Board select(int no) {
		...
		//조회 프로세스 : SQL 실행 객체 생성
		try {
			// 1. SQL 쿼리 실행 객체 생성 - PreparedStatement(stmt)
			psmt = conn.prepareStatement(sql);
			// no 매개변수의 값을 ? 파라미터에 동적으로 바인딩
			// psmt.setXXX(물음표 순서 번호, 매핑할 값);
			psmt.setInt(1, no);// 첫 번쨰 ?에 정수 no를 넣음
	...
	public int insert(Board board) {
		...
		try {
			...
			psmt.setString(1,board.getTitle() );// 첫번째 ?에 문자열 title(제목)매핑
			psmt.setString(2,board.getWriter() );// 두번째 ?에 문자열 writer(작성자)매핑
			psmt.setString(3,board.getContent() );// 세번째 ?에 문자열 content(내용)매핑
	...
	public int update(Board board) {
		...
		try {
			...
			psmt.setString(1,board.getTitle() );// 첫번째 ?에 문자열 title(제목)매핑
			psmt.setString(2,board.getWriter() );// 두번째 ?에 문자열 writer(작성자)매핑
			psmt.setString(3,board.getContent() );// 두번째 ?에 문자열 writer(작성자)매핑
			psmt.setInt(4,board.getNo() );// 세번째 ?에 문자열 content(내용)매핑
			result = psmt.executeUpdate();// SQL 실행 요청
		} catch (SQLException e) {
	...
	public int delete(int no) {
		...
		try {
			psmt =conn.prepareStatement(sql);// SQL 실행 객체 생성
			psmt.setInt(1,no );// 세번째 ?에 문자열 content(내용)매핑
			result = psmt.executeUpdate();// SQL 실행 요청
		} catch (SQLException e) {
