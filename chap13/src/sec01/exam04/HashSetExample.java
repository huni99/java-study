...
//중복된 데이터 제거가 필요한 상황에 유용 (예:ArrayList의 요소에서 중복을 제거할 때)
public class HashSetExample {
	public static void main(String[] args) {
		...
		//JVM 내부 해시 구현에 의해 우연히 정렬된 것처럼 출력된 것을 뿐 순서를 보장하지 않음
		...
		Iterator<String> iterator = set.iterator();//반복자 얻기(Iterator구현 객체)
		while (iterator.hasNext()) {//가져올 객체가 있으면 true, 없으면 false를 반환 
			String str = iterator.next();//컬렉션에서 1개의 객체를 가져옴
			// 커서는 첫 요소 이전 위치에 있음, next() 호출하면 커서가 다음 위치로 이동하면서 그 요소를 반환함
			System.out.println(str);
			//remove():next()로 반환한 요소를 제거
//		iterator.remove();
//		참고 :  remove()는 next()호출 이후에만호출 가능하며, 한 번만 가능
		}
		...
		//모든 객체를 제거하고 비움
		set.clear();
		if(set.isEmpty())System.out.println("비어있음");//컬렉션이 비어있는지 조사
		
		//HashSet과 집합
		//수학의 집합을 효율적으로 처리하게이 적합한구조
...
//		System.out.println("교집합 : "+class1);
		...
		Set<String> linkedSet = new LinkedHashSet<String>(fruitsList);//생성자 매개변수로 컬렉션이 들어감
		...
		//HashSet을 이용해 중복 제거
		Set<String> javaClass = new HashSet<String>(list);//중복제거완료
		System.out.println(javaClass);
	
		//필요 시 다시 List로 변환
		list.clear();
		list.addAll(javaClass);
		System.out.println(list);
