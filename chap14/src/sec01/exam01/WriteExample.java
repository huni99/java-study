...
//스트림의 종류
// 구분 바이트기반(1byte) 문자 기반(2byte =char)
...
public class WriteExample {
	public static void main(String[] args) throws Exception {
		...
		//출력할 바이트(0~255까지) -> 1바이트는 8비트 -> 2^8-1까지 표현가능
		...
		os.write(b);
		os.write(c);
		//write()를 한다고해서 바로 출력 스트림을 통해서 데이터가 흘러가는 것이아님
		//버퍼(임시 저장공간)에 쌓임
		//버퍼:물건을 잠시 담아두는 창고
		...
		// FileOutputStream에는 내부 버퍼가 존재하지않음
		// 즉,write() 시 곧바로 OS에 전달되어 출력
		// flush()가 필수가 아니지만, 그래도 명시적으로호출하는 것이 안전
		
		//더이상 쓰지않을 때는 출력 스트림 닫기
		os.close();//출력 스트림이 사용했던 자원들(메모리)을 풀어주기 위해
		//close() 안에서도 스트림을 닫기 전에 flush()기능을 수행함
		
		//참고 : try-with-resources
		//리소스를 자동으로 닫아주는 try 문법
		//close()를 명시적으로 호출하지 않아도, try 블로이 끝나거나 예외가 발생되는 즉시 자동으로 자원 정리
...
//		
		...
		//우리가 다루는 데이터는 크게 두 종류로 나눌수 있음
		//1. 텍스트 데이터: 사람이 읽을수 있는 문자 중심의 데이터(예:"hello","123","텍스트")
